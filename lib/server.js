...
const PACKAGE_ROOT =
  process.env.THEMETRACKER_PACKAGE_ROOT || path.dirname(__dirname);
...
const handleWebSocketUpgrade = (request, socket) => {
  ...
  socket.on("close", () => {
    console.log(
      `🔌 WebSocket client disconnected (${connectedClients.size} total)`
    );
  });
};
...
const createServer = async (serverConfig = {}) => {
  ...
  console.log(
    `🎨 Design token config: ${
      config.designTokenConfig.tokensPath || "Not found"
    }`
  );
...
const handleApiConfig = (req, res) => {
  ...
  res.end(
    JSON.stringify({
      projectRoot: PROJECT_ROOT,
      config: {
        designTokenConfig: config.designTokenConfig,
