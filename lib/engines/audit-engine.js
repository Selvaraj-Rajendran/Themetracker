...
/**
 * Cache for loaded Dew color tokens to avoid repeated file reads
 */
let dewColorsCache = null
...
const loadApprovedTailwindColors = (config) => {
  const configPath = config?.dewConfig?.tokensPath

  // Return cached result if config hasn't changed
  if (dewColorsCache && lastConfigPath === configPath) {
    return dewColorsCache
  }

  if (!configPath) {
    console.warn('Warning: No Dew config path specified, using fallback colors')
    dewColorsCache = [
      'color-text-brand',
      'color-text-primary',
  ...
  try {
    ...
    const dewColors = JSON.parse(fs.readFileSync(fullPath, 'utf8'))
    const newDewColors = Object.keys(dewColors)

    // Only log when cache is first populated
    if (!dewColorsCache) {
      console.log(`✅ Loaded ${newDewColors.length} Dew color tokens`)
    }

    dewColorsCache = newDewColors
    lastConfigPath = configPath
    return dewColorsCache
  } catch (error) {
    console.warn(`Warning: Could not load Dew colors from ${configPath}, using fallback colors`)
    dewColorsCache = [
      'color-text-brand',
      'color-text-primary',
      'color-text-secondary',
...
const isColorPropertyIgnored = (match, content, matchIndex, config) => {
  ...
  const colorPropertyPattern = /(background-color|color|border-color|fill|stroke|background):\s*$/
...
const isInsideCssVariableFallback = (match, content, matchIndex) => {
  ...
  const hasCssVarBefore = cssVarPatterns.some((pattern) => pattern.test(beforeMatch))
...
const VIOLATION_PATTERNS = [
  ...
  },
  {
    ...
    pattern: /(?:gap|top|right|bottom|left):\s*[^;]*\d+(?:\.\d+)?(?:px|rem|em)[^;]*/g,
    severity: 'medium',
    suggestion:
...
const extractViolations = (content, filePath, config, fileType) => {
  ...
  let unapprovedMatch
  while ((unapprovedMatch = tailwindChecker.unapprovedColorTokens.exec(content)) !== null) {
    const lineNumber = content.substring(0, unapprovedMatch.index).split('\n').length
    const lineContent = lines[lineNumber - 1] || ''
    ...
    violations.push({
      ...
      line: lineNumber,
      column: unapprovedMatch.index - content.lastIndexOf('\n', unapprovedMatch.index),
      context: lineContent.trim(),
      fullContext: getFullContext(lines, lineNumber - 1, 2),
      suggestion: `Use approved Dew color tokens: ${approvedColors.slice(0, 3).join(', ')}, etc.`,
  ...
  if (['css', 'scss', 'sass', 'less'].includes(fileType)) {
    const scssVarPattern = /\$[a-zA-Z-]+:\s*(?:#[0-9a-fA-F]+|rgb|rgba|[\d.]+(?:px|rem|em))/g
    let scssMatch
    while ((scssMatch = scssVarPattern.exec(content)) !== null) {
      const lineNumber = content.substring(0, scssMatch.index).split('\n').length
      const lineContent = lines[lineNumber - 1] || ''
      ...
      violations.push({
        ...
        context: lineContent.trim(),
        fullContext: getFullContext(lines, lineNumber - 1, 2),
        suggestion: 'Use CSS custom properties or import values from design tokens',
        match: scssMatch[0]
      })
    }
  }
...
const extractThemeTokens = (content, config) => {
  ...
  if (config?.dewConfig) {
    ...
    let approvedMatch
    while ((approvedMatch = checker.approvedColorTokens.exec(content)) !== null) {
...
const determineFileType = (filePath, content) => {
  ...
  const hasUIIndicators = uiIndicators.some(
    (indicator) => content.includes(indicator) || new RegExp(indicator).test(content)
  )
  ...
  const utilityIndicators = ['util', 'helper', 'service', 'api', 'hook', 'context']
  const isUtility = utilityIndicators.some(
    (indicator) =>
      fileName.toLowerCase().includes(indicator) || filePath.toLowerCase().includes(indicator)
  )
...
const calculateContextualCoverage = (content, violations, themeTokens, fileType) => {
  ...
  const baseCoverage = contentLength > 0 ? Math.max(0, 100 - violationScore * 10) : 100
  ...
  try {
    ...
    return {
      ...
      analysis: {
        ...
        recommendations: generateRecommendations(violations, themeTokens, fileType, config)
      }
    }
  } catch (error) {
