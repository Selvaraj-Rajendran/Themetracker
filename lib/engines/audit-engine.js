...
// Default violation patterns for design token-based systems
const getDefaultViolationPatterns = () => [
  {
...
/**
 * Get violation patterns based on configuration
 */
const getViolationPatterns = (config) => {
  const customViolations = config?.customViolations || {};

  // Start with default patterns
  let patterns = customViolations.overrideDefaults
    ? []
    : getDefaultViolationPatterns();

  // Remove disabled default patterns
  if (
    customViolations.disableDefaultPatterns &&
    customViolations.disableDefaultPatterns.length > 0
  ) {
    patterns = patterns.filter(
      (pattern) =>
        !customViolations.disableDefaultPatterns.includes(pattern.type)
    );
  }

  // Add custom patterns
  if (
    customViolations.enabled &&
    customViolations.patterns &&
    customViolations.patterns.length > 0
  ) {
    patterns.push(...customViolations.patterns);
  }
...
const extractViolations = (content, filePath, config, fileType) => {
  ...
  // Get violation patterns from configuration
  const violationPatterns = getViolationPatterns(config);
