...
const DEFAULT_CONFIG = {
  ...
  ignore: {
    directories: [
      "node_modules", "dist", "build", "coverage", "out", ".next", ".nuxt", ".output",
      ".git", ".svn", ".hg", ".vscode", ".idea", ".vs", ".cache", ".parcel-cache", ".webpack",
      "__tests__", "test", "tests", "spec", "specs", "stories", "docs", "documentation", ".storybook",
      "mocks", "mock", "__mocks__", "fixtures", "utils", "util", "helpers", "helper", 
      "services", "service", "api", "apis", "store", "stores", "config", "configs",
      "constants", "constant", "types", "models", "interfaces", "enums", "definitions",
      ".pnpm", ".yarn", "theme-tracker"
    ],
    filePatterns: [
      "*.test.*", "*.spec.*", "test.*", "*.test", "*.spec", "*.stories.*", "*.story.*", "stories.*",
      "*mock*", "*Mock*", "*.mock.*", "*.d.ts", "*.config.*", "*.conf.*", "index.*",
      "*.types.*", "*.model.*", "*.interface.*", "*.enum.*", "*.constants.*", "*.service.*",
      "*.util.*", "*.utils.*", "*.helper.*", "*.api.*", "*.generated.*", "*.gen.*", "*.min.*",
      "bundle.*", "chunk.*"
    ],
    pathPatterns: [
      "/test/", "/tests/", "/__tests__/", "/spec/", "/specs/", "/stories/", "/docs/", "/.storybook/",
      "/mock/", "/mocks/", "/__mocks__/", "/fixtures/", "/utils/", "/util/", "/helpers/", "/helper/",
      "/services/", "/service/", "/api/", "/apis/", "/store/", "/stores/", "/config/", "/configs/",
      "/constants/", "/constant/", "/types/", "/models/", "/interfaces/", "/enums/", "/definitions/",
      "/dist/", "/build/", "/coverage/", "/out/", "/.next/", "/.git/", "/node_modules/"
    ],
    customPatterns: []
  },
  ...
  analysis: {
    ...
    fileClassification: {
      minStylingLines: 3,
      uiComponentIndicators: ["styled", "className", "css", "emotion", "makeStyles", "useStyles", "theme"],
      utilityIndicators: ["util", "helper", "service", "api", "config", "constant", "type", "interface", "enum"]
    },
    coverage: {
      expectedCoverage: {
...
class ConfigLoader {
  ...
  async loadConfig() {
    ...
    if (config.sourceDirectories.autoDetect) {
      config.sourceDirectories = await this.autoDetectSourceDirectories(config.sourceDirectories)
    }

    // Auto-detect dew config if not specified
    if (!config.dewConfig.tokensPath) {
      config.dewConfig.tokensPath = await this.autoDetectDewConfig(config.dewConfig.fallbackPaths)
    }
  ...
  async autoDetectSourceDirectories(sourceConfig) {
    ...
    if (detectedDirs.length === 0) {
      console.warn(`⚠️  No source directories found, falling back to scanning project root`)
      return { ...sourceConfig, include: ['.'] }
    }

    console.log(`🔍 Auto-detected source directories: ${detectedDirs.join(', ')}`)
    return { ...sourceConfig, include: detectedDirs }
  }

  /**
   * Auto-detect Dew Design System configuration
   */
  async autoDetectDewConfig(fallbackPaths) {
    for (const dewPath of fallbackPaths) {
      ...
      if (fs.existsSync(fullPath)) {
        console.log(`🎨 Found Dew config at: ${dewPath}`)
        return dewPath
      }
    }

    console.warn(`⚠️  Dew Design System config not found. Checked paths:`)
    fallbackPaths.forEach(p => console.warn(`   - ${p}`))
    console.warn(`   Consider creating a config file or installing @dew/design-system`)
  ...
  validateConfig(config) {
    ...
    if (!Array.isArray(config.sourceDirectories.include)) {
      config.sourceDirectories.include = [config.sourceDirectories.include].filter(Boolean)
    }
    ...
    if (typeof config.server.port !== 'number' || config.server.port < 1000 || config.server.port > 65535) {
      console.warn(`⚠️  Invalid server port ${config.server.port}, using default 3001`)
  ...
  shouldIgnoreDirectory(dirName) {
    ...
    return config.ignore.directories.some(pattern => 
  ...
  async createSampleConfig() {
    ...
    const templatePath = path.join(__dirname, '../../templates/themetracker.config.js')
    ...
    try {
      ...
      console.log(`💡 Edit this file to customize ThemeTracker for your project`)
      return true
    } catch (error) {
