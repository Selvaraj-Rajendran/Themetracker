...
class ConfigLoader {
  ...
  async loadConfig() {
    ...
    if (!config.designTokenConfig.tokensPath) {
      config.designTokenConfig.tokensPath =
        await this.autoDetectDesignTokenConfig(
          config.designTokenConfig.fallbackPaths
        );
    }
   ...
   */
  async autoDetectDesignTokenConfig(fallbackPaths) {
    for (const tokenPath of fallbackPaths) {
      const fullPath = path.join(this.projectRoot, tokenPath);
      if (fs.existsSync(fullPath)) {
        console.log(`🎨 Found config at: ${tokenPath}`);
        return tokenPath;
      }
    }
