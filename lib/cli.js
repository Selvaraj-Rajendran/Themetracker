...
const showBanner = () => {
  ...
  console.log(`🎨 ThemeTracker v${packageJson.version}`);
  console.log("━".repeat(50));
  console.log("Real-time theme auditing for design token systems");
  console.log("");
};
...
const startThemeTracker = async () => {
  ...
  try {
    ...
    console.log(
      `🌐 Server will start on: http://${config.server.host}:${config.server.port}`
    );

    if (!config.designTokenConfig.tokensPath) {
      console.log("");
      console.log("⚠️  Design token configuration not found!");
      console.log('💡 Run "themetracker init" to create a sample config file');
      console.log("🔧 Or ensure design tokens are available in your project");
      console.log("");
    }
  } catch (error) {
...
const initConfig = async () => {
  ...
  try {
    ...
    if (created) {
      ...
      console.log(
        "   completely ignore violations for specific CSS properties"
      );
      console.log("");
      console.log("📝 Next steps:");
      console.log(
        "1. Edit themetracker.config.js to match your project structure"
      );
      console.log("2. Ensure design tokens are available");
      console.log('3. Run "themetracker start" to begin auditing');
      console.log("");
      console.log(
        "💡 Tip: ThemeTracker will auto-detect most settings if not configured"
      );
      ...
      console.log(
        "🎨 Note: Enhanced CSS variable fallback support is now active!"
      );
      console.log(
        "🎯 Note: CSS properties ignore list feature is now available!"
      );
    }
  } catch (error) {
...
const showInfo = async () => {
  ...
  try {
    ...
    console.log("\n🔧 Configuration:");
    console.log(
      `   Config file: ${config._configFile || "Auto-detected defaults"}`
    );
    console.log(
      `   Source directories: ${config.sourceDirectories.include.join(", ")}`
    );
    console.log(
      `   Design tokens: ${config.designTokenConfig.tokensPath || "Not found"}`
    );
...
/**
 * Start ThemeTracker MCP Server
 */
const startMCPServer = async () => {
  showBanner();
  console.log("🔌 Starting ThemeTracker MCP Server...");

  const projectRoot = process.cwd();

  try {
    const { ThemeTrackerMCPServer } = require("./mcp/theme-mcp-server");
    const mcpServer = new ThemeTrackerMCPServer(projectRoot);

    console.log("📂 Project:", path.basename(projectRoot));
    console.log("🎯 MCP Server ready for AI integration");
    console.log("");
    console.log("💡 To integrate with Cursor:");
    console.log("   Add this to your Cursor settings:");
    console.log("   {");
    console.log('     "mcpServers": {');
    console.log('       "themetracker": {');
    console.log('         "command": "npx",');
    console.log('         "args": ["themetracker", "mcp"],');
...
const showVersion = () => {
  ...
  console.log("Real-time theme auditing for design token systems");
  console.log("");
  console.log("📖 Documentation: https://github.com/your-org/themetracker");
  console.log(
...
const showHelp = () => {
  ...
  console.log(
    "  themetracker start                    # Start with auto-detection"
  );
  console.log("  themetracker start --port 3002        # Start on custom port");
  console.log(
    "  themetracker mcp                      # Start MCP server for AI"
  );
  ...
  console.log("  ✅ Design token system integration");
  ...
  console.log("  🤖 AI integration via MCP protocol");
  console.log("");
  console.log("Access:");
  console.log("  🌐 Dashboard: http://localhost:3001");
  console.log("  📊 Summary: http://localhost:3001/src/summary.html");
  console.log('  🔌 MCP: Use "themetracker mcp" for AI integration');
  console.log("");
